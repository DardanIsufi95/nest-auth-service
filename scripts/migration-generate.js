...
if (!migrationName) {
  ...
  console.error('Usage: node scripts/migration-generate.js <migration-name>');
  process.exit(1);
}
...
// Execute TypeORM CLI command using npm run
const typeorm = spawn(
  'npm', 
  [
    'run',
...
typeorm.on('close', (code) => {
  if (code === 0) {
    console.log(`\nMigration ${formattedName} generated successfully!`);
  } else {
    console.error(`\nFailed to generate migration with exit code ${code}`);
  }
}); 